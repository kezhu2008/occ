...
**Lens 2 — frontloading + escalation discipline.** The access pre-flight names **every** founder-only dependency up front (🔑 IBKR Flex token, 🔑 quote key, ✋ Apple/APNs signing, 💰 AWS account) and each has a `verify:` gate — never an assumed-working key. And no direction-changing call was silently auto-decided: **Path A vs Path B**, pricing, public/brand naming, the licensed-quote-feed bet, spend that locks a future wave must be *escalated by the CEO*, staged ready-to-flip — not decided in the plan. An auto-taken Path B or an auto-picked public price is an autonomy-trap FAIL.
